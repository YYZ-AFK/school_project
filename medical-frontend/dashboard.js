...
function renderPieChart(data) {
  ...
  try {
    ...
    chart.setOption({
      ...
      legend: { orient: 'vertical', left: 8, top: 'center', itemWidth: 12, itemHeight: 12, textStyle: { color: '#607472' } },
      series: [{
        type: 'pie',
        radius: ['42%', '68%'],
        center: ['58%', '50%'],
...
async function loadBaseInfo() {
  ...
  } catch (e) { console.warn("鍖荤敓瀛楀吀鍔犺浇澶辫触", e); }
}
...
async function loadDoctors() {
  ...
  } catch (e) { console.warn("鍖荤敓鍔犺浇澶辫触", e); }
}
...
function initDoctorEvents() {
  ...
  el("searchButton")?.addEventListener("click", async () => { state.pn = 1; await loadDoctors(); });
  el("refreshButton")?.addEventListener("click", loadDoctors);
  el("prevPage")?.addEventListener("click", async () => { if (state.pn > 1) { state.pn--; await loadDoctors(); } });
  el("nextPage")?.addEventListener("click", async () => { if (state.pn * state.size < state.total) { state.pn++; await loadDoctors(); } });
}
...
async function loadDrugs() {
  try {
    ...
    const mock = [
      { drugId: 12650466, drugImg: LOCAL_DRUG_IMAGES_BY_ID["12650466"], drugName: "复方感冒灵颗粒", drugSales: [{ saleName: "老百姓大药房" }], drugPublisher: "管理员" },
      { drugId: 12650467, drugImg: LOCAL_DRUG_IMAGES_BY_ID["12650467"], drugName: "连花清瘟胶囊", drugSales: [{ saleName: "好药师大药房" }], drugPublisher: "管理员" },
      { drugId: 12650468, drugImg: LOCAL_DRUG_IMAGES_BY_ID["12650468"], drugName: "布洛芬混悬液", drugSales: [{ saleName: "仁和堂大药房" }], drugPublisher: "管理员" },
    ];
...
function initDrugEvents() {
    ...
    if (deleteBtn) {
      ...
      if (confirm(`确定删除“${name}”吗？`)) {
        try {
          ...
          await loadDrugs();
        } catch (err) { setMessage("drugMessage", `操作失败：${err.message}`); }
      }
      return;
    }
  ...
  el("drugSearchButton")?.addEventListener("click", async () => { state.drugPn = 1; await loadDrugs(); });
  el("drugRefreshButton")?.addEventListener("click", loadDrugs);
  el("drugPrevPage")?.addEventListener("click", async () => { if (state.drugPn > 1) { state.drugPn--; await loadDrugs(); } });
  el("drugNextPage")?.addEventListener("click", async () => { if (state.drugPn * state.drugSize < state.drugTotal) { state.drugPn++; await loadDrugs(); } });
}
...
async function loadPolicies() {
  try {
    ...
    const mock = [
      { id: 1, title: "城乡居民基本医疗保险门诊统筹制度备案", message: "全体参保居民均享受普通门诊待遇。", cityId: 1 },
...
function initPolicyEvents() {
  el("policyForm")?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      state.policyPn = 1;
      await loadPolicies();
    } catch (err) { setMessage("policyMessage", `操作失败：${err.message}`); }
  });
  ...
  el("policySearchButton")?.addEventListener("click", async () => { state.policyPn = 1; await loadPolicies(); });
  el("policyRefreshButton")?.addEventListener("click", loadPolicies);
  el("policyPrevPage")?.addEventListener("click", async () => { if (state.policyPn > 1) { state.policyPn--; await loadPolicies(); } });
  el("policyNextPage")?.addEventListener("click", async () => { if (state.policyPn * state.policySize < state.policyTotal) { state.policyPn++; await loadPolicies(); } });
}
...
async function loadCompanyPolicies() {
  try {
    ...
    const mock = [
      { id: 1, title: "药品上市许可持有人制度", message: "上市许可持有人和生产许可持有人可以是同一主体。", companyId: 1 },
...
function initCompanyPolicyEvents() {
  el("companyPolicyForm")?.addEventListener("submit", async (e) => {
    ...
    } catch (err) { setMessage("companyPolicyMessage", `操作失败：${err.message}`); }
  });
  ...
  el("companyPolicySearchButton")?.addEventListener("click", async () => { state.companyPolicyPn = 1; await loadCompanyPolicies(); });
  el("companyPolicyRefreshButton")?.addEventListener("click", loadCompanyPolicies);
  el("companyPolicyPrevPage")?.addEventListener("click", async () => { if (state.companyPolicyPn > 1) { state.companyPolicyPn--; await loadCompanyPolicies(); } });
  el("companyPolicyNextPage")?.addEventListener("click", async () => { if (state.companyPolicyPn * state.companyPolicySize < state.companyPolicyTotal) { state.companyPolicyPn++; await loadCompanyPolicies(); } });
}
...
function initMaterialEvents() {
  el("materialForm")?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      state.materialPn = 1;
      await loadMaterials();
    } catch (err) { setMessage("materialMessage", `操作失败：${err.message}`); }
  });
  ...
  el("materialSearchButton")?.addEventListener("click", async () => { state.materialPn = 1; await loadMaterials(); });
  el("materialRefreshButton")?.addEventListener("click", loadMaterials);
  el("materialPrevPage")?.addEventListener("click", async () => { if (state.materialPn > 1) { state.materialPn--; await loadMaterials(); } });
  el("materialNextPage")?.addEventListener("click", async () => { if (state.materialPn * state.materialSize < state.materialTotal) { state.materialPn++; await loadMaterials(); } });
}
  ...
  drug: {
    ...
    getPayload: (d) => {
      const saleIds = d.saleIds && d.saleIds.trim()
        ...
        : [];
      return { drugName: d.name, drugInfo: d.info, drugEffect: d.effect, drugImg: d.img || "", drugPublisher: d.publisher || "管理员", saleIds: saleIds };
    },
    getEditData: (r) => ({ name: r.name || '', info: r.info || '', effect: r.effect || '', img: r.img || '', publisher: r.publisher || '', saleIds: r.saleIds || '' }),
...
function initEditEvents() {
  ...
  el("editForm")?.addEventListener("submit", async (e) => {
    ...
    if (!config) return;
    const fieldData = {};
    config.fields.forEach(f => { fieldData[f.key] = getEditFieldValue(f.key); });
    ...
    try {
      ...
      const refreshMap = { drug: loadDrugs, policy: loadPolicies, companyPolicy: loadCompanyPolicies, material: loadMaterials, company: loadCompanies };
...
async function loadSales() {
  try {
    ...
    const mock = [
      { saleId: 1, saleName: "老百姓大药房", salePhone: "028-83510000", address: "成都市成华区建设路", lng: 104.101, lat: 30.673 },
      { saleId: 2, saleName: "好药师大药房", salePhone: "028-84858000", address: "成都市龙泉驿区龙泉街道", lng: 104.274, lat: 30.556 },
      { saleId: 3, saleName: "仁和堂大药房", salePhone: "028-86630000", address: "成都市青羊区人民东路", lng: 104.066, lat: 30.659 },
      { saleId: 4, saleName: "天士力大药房", salePhone: "028-86270000", address: "成都市青羊区宽窄巷子", lng: 104.048, lat: 30.664 },
      { saleId: 5, saleName: "益丰大药房", salePhone: "028-85560000", address: "成都市武侯区武侯祠大街", lng: 104.055, lat: 30.644 },
    ];
...
function initSaleEvents() {
  el("saleSearchButton")?.addEventListener("click", async () => { state.salePn = 1; await loadSales(); });
  el("saleRefreshButton")?.addEventListener("click", loadSales);
  el("salePrevPage")?.addEventListener("click", async () => { if (state.salePn > 1) { state.salePn--; await loadSales(); } });
  el("saleNextPage")?.addEventListener("click", async () => { if (state.salePn * state.saleSize < state.saleTotal) { state.salePn++; await loadSales(); } });
  ...
  el("addSaleForm")?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (msgEl) { msgEl.textContent = "新增成功"; msgEl.style.color = "#237b7b"; }
      setTimeout(() => {
      ...
      const msgEl = el("addSaleMessage");
      if (msgEl) { msgEl.textContent = `操作失败：${err.message}`; msgEl.style.color = "#b44735"; }
    }
  });
