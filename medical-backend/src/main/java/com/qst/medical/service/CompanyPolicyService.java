...
public class CompanyPolicyService {
     ...
     * 分页、关键字查询医药公司政策信息
     */
    public Result getAllPolicyWithPage(CompanyPolicyParam param) {
        if (param.getSize() == 0) {
            param.setSize(1);
        }
     ...
     */
    @Transactional
    public Result savePolicy(CompanyPolicyParam param) {
     ...
     */
    @Transactional
    public Result updatePolicy(Long id, CompanyPolicyParam param) {
        CompanyPolicyEntity entity = new CompanyPolicyEntity();
        BeanUtils.copyProperties(param, entity);
     ...
     */
    @Transactional
    public Result deletePolicy(Long id) {
